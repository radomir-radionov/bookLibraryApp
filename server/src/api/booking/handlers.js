...
const {
  ...
  EXPIRED_BOOKING_DELETE_ERROR,
} = errorText;
const { BOOKING_DELETED_SUCCESS, EXPIRED_BOOKING_DELETED_SUCCESS } =
  responseText;

const createBooking = async (ctx, next) => {
  const bookingData = ctx.request.body;
  console.log(1);
  const bookingByUserId = await Booking.findAll({
    where: { userId: bookingData.userId },
  });

  const bookingsByBookId = await Booking.findAll({
  ...
  const book = await Book.findOne({
    ...
    attributes: {
      exclude: ['issueYear', 'categories', 'createdAt', 'updatedAt'],
    },
  });
